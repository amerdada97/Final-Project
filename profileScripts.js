...
function getUser(){
    ...
    .then(function(response){
        ...
        document.getElementById("posts-count").innerHTML = `${user.posts_count} Posts`
        document.getElementById("comments-count").innerHTML = `${user.comments_count} Comments`
    })
}
...
function getPosts(){
    ...
    for (post of posts) {
      ...
      for (tag of post.tags){
        let tagsContent = 
        `
        <button class="btn btn-sm rounded-5" style="background-color:grey;color:white;">${tag.name}</button>
        `
